...
namespace NodeCanvas.DialogueTrees.UI.Examples
{
    ...
    public class DialogueUGUI : MonoBehaviour, IPointerClickHandler
    {
        ...
        // 修改点 1：原来叫 originalSubsPosition，现在改成记录 UI 锚点坐标
        private Vector2 originalSubsAnchoredPosition;
        ...
        void Hide()
        {
            ...
            // 修改点 2：原来是 subtitlesGroup.transform.position，这是世界坐标，会导致窗口变化后 UI 偏移
            originalSubsAnchoredPosition = subtitlesGroup.anchoredPosition;
        }
        ...
        IEnumerator Internal_OnSubtitlesRequestInfo(SubtitlesRequestInfo info)
        {
            ...
            // 修改点 3：原来是 subtitlesGroup.position，这是世界坐标；UI 应该恢复 anchoredPosition
            subtitlesGroup.anchoredPosition = originalSubsAnchoredPosition;
            ...
            if (waitForInput)
            {
                ...
                while (!(Input.GetMouseButtonDown(0) || Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.KeypadEnter) || Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.E)))
                {
                    yield return null;
                }
                waitInputIndicator.gameObject.SetActive(false);
