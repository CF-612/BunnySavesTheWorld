...
- 主菜单现在将新游戏和续玩拆成两个按钮：`StartNewGame()` 清空进度并播放开场剧情，`ContinueGame()` 只在存在存档时加载最近场景；`StartGame()` 仅作为旧 UnityEvent 的兼容别名。

## 2026-09-01 未提交的缺陷修复

- **玩家销毁期间的输入锁清理**：对话和 NPC 适配器只释放自己实际取得的锁，并使用 Unity 对象有效性判断；`Player` 在 `OnDisable`/`OnDestroy` 阶段不再访问已经销毁的输入对象，避免异步场景切换时出现 `Player` 已销毁异常。
- **开场剧情结束时的主菜单闪现**：旧版剧情结束后直接同步加载场景，新版先隐藏最后一页再异步启动透明黑幕，二者之间可能露出主菜单底层一帧。现已让 `StorySequence` 在没有下一页时保留当前页，直到 `SceneTransitionService` 的黑幕覆盖后再卸载旧场景。

以上改动均保持未提交状态，需在 Unity Play Mode 中验证后再提交。
...
8. 从主菜单播放完整开场剧情，确认最后一页直接渐黑进入 `Home_Light`，期间不闪回主菜单背景。
