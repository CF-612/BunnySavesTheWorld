...
在保留现有纯 C# 玩家状态机、Rigidbody2D、Input System、风区、滑翔、跺脚、管道和单向平台下落的基础上，将基础移动调整为偏灵敏的平台动作手感，并为普通跳跃增加操作容错和可变高度。同时彻底删除原有蓄力跳跃，避免长按空格导致延迟起跳或进入独立蓄力状态。
...
本次不包含二段跳、风区参数化重构、动画播放速度联动、脚步声精细联动、调试 UI、额外特效或输入绑定扩展。
...
- [x] 既有风区兼容：风区活跃时继续使用当前直接速度计算，不新增或修改风区参数体系。
...
- `Player`：移动参数、跳跃参数和运行时跳跃上下文的唯一所有者。
...
- `Player.controller`：保留普通 `jumpFall` BlendTree，删除专用于蓄力跳的内容。
...
1. 用户操作：角色进入已配置的风区并移动或跳跃。
2. 系统响应：有风时继续使用现有地面/空中风力目标速度和朝向防抖逻辑。
3. 预期结果：风区行为不因基础移动优化而被重新定义；本需求不评价或调整风区自身手感。
...
- 不新增 `PlayerMovementProfile`、二段跳计数、风区移动倍率、Animator 速度参数或调试面板。
- 运动参数稳定后，如仍存在动画与脚步频率不匹配，应另立表现调校需求。
...
- `Assets/Resources/Scripts/Player/PlayerStates/Player_MoveState.cs`：地面移动与既有地面风区逻辑。
- `Assets/Resources/Scripts/Player/PlayerStates/Player_idleState.cs`：地面减速。
...
- `Assets/Resources/Animations/Player/Player.controller`：普通跳跃动画状态机。
